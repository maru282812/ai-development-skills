...
description: >-
  ...
  トリガー例:
  「ループしちゃって」「ループして」「ループ入れて」「ループ化して」「ループ回せるようにして」
  「製造ループを入れて」「ループエンジニアリングを導入して」「Loop Engineering を入れて」
  「Factory を bootstrap して」「factory 入れて」「このプロジェクトに loop 基盤を設置して」
  「製造工程を整えて」「製造ラインを整えて」「VERIFY.md を作って」「検証レシピを作って」
  「新規プロジェクトの製造ラインを整備して」「このリポジトリを製造できる状態にして」。
  「ループ」という語だけの依頼でも、対象がプロジェクト/リポジトリの開発体制であればこのスキルに入る
  （定期実行のスケジューリングを求める `/loop` とは別物。定期実行の設定はそちらへ）。
