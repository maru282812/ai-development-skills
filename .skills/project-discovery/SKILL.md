...
description: >-
  ...
  各フェーズは [[discovery-planner]] が仮置きで完走させてフェーズ末にだけ一括確認し（フェーズ途中で止めない＝/loop 向き）、[[discovery-auditor]] で横断監査する。
...
# Phase完了基準（Exit Gate）と確信度

各 Phase は「終わったか」をチェックリストで判定できなければループが止まらない／早すぎる引き渡しが起きる。フェーズ末に必ずこの **Exit Gate** を全項目チェックする。`/loop` はこの □ を機械的に読んで「次フェーズへ前進してよいか」を判定する。

```text
## Phase Exit Gate: Phase<N> <名前>
□ そのフェーズの必須成果物がすべて生成/更新されている（フェーズ表の「成果物」列）
□ 必須項目カバー率 <x/y> が基準を満たす
□ 人間必須の仮置き（事業意図/課金/対象ユーザー/提供価値/責任範囲/データ所有権/法務方針）が
   assumption のまま未昇格で残っていない（残るなら「継続」。承認済みなら昇格して □ 可）
□ open-questions.md にこのフェーズ起因の期限切れ・未回答の人間必須がない
□ 下流（画面設計・実装）をブロックする前段との矛盾がない（discovery-auditor 軽量モードで確認済み）
- 確信度: 高 / 中 / 低
- 判定: 完了（全□充足→次フェーズ） / 継続（未充足あり） / 差し戻し（→ どのPhaseへ）
```

- **自動充填可の仮置き**（技術慣習・デフォルト・前段から導ける）は、フェーズ末の確認表でユーザー承認されれば □ を満たして通してよい。
- **人間必須の仮置き** が未昇格のまま残るフェーズは「完了」にしない（[[discovery-auditor]] が Critical で弾く）。浅い暫定として明示的に通す場合のみ例外、その旨を decisions.md に記録。
...
```text
フェーズ着手（＝1ゴール）
 → [[discovery-planner]]: フェーズを仮置き(assumption)で埋め切って完走させる
      ・自動充填可 → assumption で前進（途中で止めない）
      ・人間必須 → 要確認に上げる（断定しない）
 → [[discovery-auditor]] 軽量モード: 未昇格の人間必須仮置き / 直近の矛盾 / 幽霊をスキャン
 → Phase Exit Gate（□チェックリスト）を判定
 → ★フェーズ末で停止: 成果物 ＋ [仮置き一括確認表] ＋ [人間必須の要確認] を一括提示
 → ユーザーが一括で承認/上書き（＝再開トリガー）
 → 次フェーズ（新ゴール）へ。全フェーズ後に [[discovery-auditor]] 全体モードで横断監査
 → 差し戻し or 引き渡し
```

## Discovery Loop プロトコル（`/loop` の使い方）

要件定義書づくりは **人間の回答がボトルネック** なので、時間ループ（`/loop 10m ...`）は空回りする。使うなら **間隔なしのセルフペース `/loop`** を「フェーズ完走→フェーズ末ゲートで停止」で回す。

**停止規約（厳守）**
- 1イテレーション ＝ **現フェーズ1個を Exit Gate まで完走** させる単位。フェーズ途中では止めない。
- フェーズ末で **人間必須の要確認が残る／Exit Gate 未充足** なら、**`ScheduleWakeup` を呼ばずに終了**（＝ループ終了）。再開トリガーはタイマーではなく **ユーザーの承認/回答**。
- フェーズ末で **要確認が空・Exit Gate 全充足** のときに限り、ユーザー承認を得てから **次フェーズの埋め込みに自動で入ってよい**（次イテレーションを予約 or 連続実行）。
- 全フェーズ通過後は `/loop` を終了し、[[discovery-auditor]] 全体モード→引き渡しへ。

**起動文の雛形**
```text
/loop
project-discovery の Discovery Loop プロトコルで要件定義を進めて。
現在フェーズを discovery-planner で仮置き完走させ、discovery-auditor 軽量モードで点検し、
Phase Exit Gate を判定。フェーズ末に「仮置き一括確認表＋人間必須の要確認」を出して停止して。
人間必須が残る/ゲート未充足なら ScheduleWakeup を呼ばずに終了。全フェーズ通過で終了。
```
